...
public class XMLMapperBuilder extends BaseBuilder {
    ...
    public void parse() {
        /*
         * 这里先判断一下resource(如com/loserico/mybatis/eden/mapper/UserMapper.xml)是否已经被加载过,
         * 即Configuration的loadedResources里面是否包含这个XXXMapper.xml
         */
    ...
    private void configurationElement(XNode context) {
        try {
            ...
            /*
             * 这个是解析SQL片段, 不是实际的SQL语句
             */
            sqlElement(context.evalNodes("/mapper/sql"));
            /*
             * 解析mapper节点下的所有 select/insert/update/delete 节点
             * 解析到 Configuration#mappedStatements
    ...
    private void cacheElement(XNode context) {
        if (context != null) {
            //解析cache节点的type属性, 即缓存的实现类, 默认是 org.apache.ibatis.cache.impl.PerpetualCache
            String type = context.getStringAttribute("type", "PERPETUAL");
            //根据type的值获取class类型
            Class<? extends Cache> typeClass = typeAliasRegistry.resolveAlias(type);
            //获取缓存过期策略: 默认是LRU
            String eviction = context.getStringAttribute("eviction", "LRU");
            //LRU对应的实现类是: org.apache.ibatis.cache.decorators.LruCache
            Class<? extends Cache> evictionClass = typeAliasRegistry.resolveAlias(eviction);
            /*
             * flushInterval(刷新间隔)
             * 该属性可以被设置为任意的正整数, 设置的值应该是一个以毫秒为单位的合理时间量
             * 默认没有设置, 也就是没有刷新间隔, 缓存仅仅会在调用语句时刷新
             */
            Long flushInterval = context.getLongAttribute("flushInterval");
            /*
             * size(引用数目)
             * 该属性可以被设置为任意正整数, 要注意欲缓存对象的大小和运行环境中可用的内存资源, 默认值是 1024。
             */
            Integer size = context.getIntAttribute("size");
            /*
             * 该属性可以被设置为 true 或 false
